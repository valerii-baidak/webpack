...
> ❗️ Мы поддерживаем только последние **`LTS-версии Node.js`**, указанные в этой инструкции. Мы не даём гарантий работы на других версиях Node. Если у тебя не работает, в первую очередь проверь версию Node.js!
...
```bash
git clone https://github.com/Lectrum/webpack-offline-intensive.git
```

7. Перейди в проект, выполнив команду:

```bash
cd webpack-offline-intensive
```
...
9. Открой файл **`package.json`**, в поле **`version`** должно быть такое значение **`"0.0.1"`**;
